...
namespace Teeyoot.Payouts.Controllers
{
    ...
    public class TranzactionController : Controller
    {
        ...
        public ActionResult Index(PagerParameters pagerParameters, PayoutsViewModel adminViewModel, string filter = "")
        {
            ...
            {
                if (filter == "1")
                    list = payouts.Select(s => new History { Id = s.Id, Date = s.Date, Description = s.Description, Amount = s.Amount, IsPlus = s.IsPlus, UserId = s.UserId, Status = s.Status, CampainID = s.CampaignId }).Where(s => s.Status == "Pending" && s.IsPlus == true).ToList();
                else if (filter == "2")
                    list = payouts.Select(s => new History { Id = s.Id, Date = s.Date, Description = s.Description, Amount = s.Amount, IsPlus = s.IsPlus, UserId = s.UserId, Status = s.Status, CampainID = s.CampaignId }).Where(s => s.Status == "Completed" && s.IsPlus == true).ToList();
                else
                    list = payouts.Select(s => new History { Id = s.Id, Date = s.Date, Description = s.Description, Amount = s.Amount, IsPlus = s.IsPlus, UserId = s.UserId, Status = s.Status, CampainID = s.CampaignId }).Where(s => s.IsPlus == true).ToList();
                ...
                var entriesProjection = list.Select(e =>
                {
                    return Shape.FaqEntry(
                        ...
                        UserId: e.UserId,
                        IsPlus: e.IsPlus,
                        CampaignAlias: _campService.GetCampaignById(e.CampainID).Alias,
                        CampaignName: _campService.GetCampaignById(e.CampainID).Title,
        ...
        public ActionResult Payouts(PagerParameters pagerParameters, PayoutsViewModel adminViewModel, string filter = "")
        {
            ...
            {
                if (filter == "1")
                    list = payouts.Select(s => new History { Id = s.Id, Date = s.Date, Description = s.Description, Amount = s.Amount, IsPlus = s.IsPlus, UserId = s.UserId, Status = s.Status }).Where(s => s.Status == "Pending" && s.IsPlus == false).ToList();
                else if (filter == "2")
                    list = payouts.Select(s => new History { Id = s.Id, Date = s.Date, Description = s.Description, Amount = s.Amount, IsPlus = s.IsPlus, UserId = s.UserId, Status = s.Status }).Where(s => s.Status == "Completed" && s.IsPlus == false).ToList();
                else
                    list = payouts.Select(s => new History { Id = s.Id, Date = s.Date, Description = s.Description, Amount = s.Amount, IsPlus = s.IsPlus, UserId = s.UserId, Status = s.Status }).Where(s => s.IsPlus == false).ToList();
