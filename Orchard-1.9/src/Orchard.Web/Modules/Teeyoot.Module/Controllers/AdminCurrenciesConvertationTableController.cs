...
namespace Teeyoot.Module.Controllers
{
    ...
    public class AdminCurrenciesConvertationTableController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Edit(CurrencyExchangeMatrixViewModel viewModel, string currency)
        {
            int x = 0;
                ...
                foreach (var newExchange in column)
                {
                    if (x != y)
                    {
                        var record = _currencyExchangeRepository.Table
                                ...
                                .First();
                        var recordReverce = _currencyExchangeRepository.Table
                                .Where(s => (s.CurrencyTo.Id == newExchange.CurrencyFromId)
                                    && (s.CurrencyFrom.Id == newExchange.CurrencyToId))
                                .First();
                        ...
                        record.RateForSeller = newExchange.RateForBuyer;


                        //recordReverce.CurrencyFrom = _currencyRepository.Get(newExchange.CurrencyToId);
                        //recordReverce.CurrencyTo = _currencyRepository.Get(newExchange.CurrencyFromId);
                        //recordReverce.RateForBuyer = 1 / newExchange.RateForBuyer;
                        //recordReverce.RateForSeller = 1 / newExchange.RateForBuyer;


                        _currencyExchangeRepository.Update(record);
                        _currencyExchangeRepository.Update(recordReverce);
                    }
                    //}
                    y++;
                }
