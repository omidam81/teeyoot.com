...
namespace Teeyoot.Module.Controllers
{
    ...
    public class AdminUserController : Controller
    {
        ...
        private readonly IRepository<UserRolesPartRecord> _userrolrepository;
        ...
        public ActionResult EditUser(int userId)
        {
            ...
            viewModel.SelectedRolsd = _userrolrepository.Table.Where(aa => aa.UserId == user.Id).Select(aa => aa.Role.Id).ToArray();
            viewModel.ReceiveEmail = (teeyootUser == null) ? false : teeyootUser.ReceiveEmail;

            ViewBag.Roles =  _roleRepository.Table
                .Select(r => new RoleItemViewModel
                {
                    Id = r.Id,
                    Name = r.Name
        ...
        public ActionResult EditUser(EditUserViewModel model)
        {
            ...
            var teeyootUser = user.As<TeeyootUserPart>();
            if (teeyootUser != null) teeyootUser.ReceiveEmail = model.ReceiveEmail;
            ...
            var roles = _userrolrepository.Fetch(aa => aa.UserId == model.UserId);
            foreach (var rol in roles)
                _userrolrepository.Delete(rol);


            if (model.SelectedRolsd != null && model.SelectedRolsd.Count() > 0)
            {
                foreach (int role in model.SelectedRolsd)
                {
                    _userrolrepository.Create(new UserRolesPartRecord()
                    {
