...
namespace Teeyoot.Module.Services
{
    public class MailChimpSettingsService : IMailChimpSettingsService
    {
        ...
        {
            var settings = _contentManager.Query<MailChimpSettingsPart, MailChimpSettingsPartRecord>(VersionOptions.Latest).Where(fe => fe.Culture == culture);
            if (settings.Count() <= 0) culture = "en-MY";
