function resizeME() {
    app.state.w = $(".zone-content").width();
    app.state.h = $(".slide_active .step").height();

    var slides = $('.Slides > div');
    slides.css({ width: app.state.w + 'px', height: app.state.h + 100 + 'px' });

    $('.SlideContainer').height(app.state.h + 100);
};
...
window.onload = function initWizard() {
    ...
    $("#butAdd").click(function addElement() {
        ...
        var front = (design.products.images[productId].printable_front_height / design.products.images[productId].printable_front_width).toFixed(2);
        var back = (design.products.images[productId].printable_back_height / design.products.images[productId].printable_back_width).toFixed(2);
        if ((front != coeff.toFixed(2)) || (back != coeffBack.toFixed(2))) {
        ...
        window.count = 1; //parseInt(slider.noUiSlider.get());
        ...
        h6Cost.innerHTML = "";
        h4CostProfRm.innerHTML = " " + window.OldCurrency;
        h4CostProfText.innerHTML = "Minimum Selling Price: ";
        h4CostProfFloat.innerHTML = window.ConvertPirce2(prdc, "USD", window.OldCurrency).toFixed(2);
        $(h4CostProfFloat).data("original-price", prdc.BaseCost);
        ...
        h4ProfRm.innerHTML = window.OldCurrency;
        ...
        spanPrice.innerHTML = window.OldCurrency; //window.currency;
        ...
        inpPrice.value = window.ConvertPirce2(prdc.Price, "USD", window.OldCurrency).toFixed(2);
        //window.Convert prdc.Price.toFixed(2);
        $(inpPrice).data("original-price", prdc.Price);
        ...
        h4Profit.innerHTML = window.ConvertPirce2(chenges, "USD", window.OldCurrency).toFixed(2);
        ...
        $inp.change(function () {
            xPrice = parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10);
            xPrice = window.ConvertPirce2(xPrice, window.OldCurrency, "USD");
            var price = xPrice - prdc.BaseCost;

            xPrice = parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10);
            xPrice = window.ConvertPirce2(xPrice, window.OldCurrency, "USD");

            prdc.Price = xPrice;//parseFloat(xPrice);  //parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10).toFixed(2);

            h4Profit.innerHTML = window.ConvertPirce2(price, "USD", window.OldCurrency).toFixed(2); // price.toFixed(2);

            $(h4CostProfFloat).data('original-price', prdc.BaseCost);

            $(h4CostProfFloat).text(window.ConvertPirce2(prdc.BaseCost, "USD", window.OldCurrency).toFixed(2));
            $inp.data('original-price', prdc.Price);
        ...
        function profSaleProd() {
            ...
            var basePrice = window.ConvertPirce2(prdc.BaseCost, "USD", window.OldCurrency);

            var price = parseFloat(parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10) - basePrice);

            prdc.Price =
                window.ConvertPirce2(parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10), window.OldCurrency, "USD");

                //parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10);

            h4Profit.innerHTML = price; //window.ConvertPirce2(price, "USD", window.OldCurrency);
            ...
            //h4CostProfFloat.innerHTML = prdc.BaseCost.toFixed(2);
            if (typeof (prdc.Price) == "string")
                prdc.Price = parseFloat(prdc.Price);

            $(inpPrice).data('original-price', prdc.Price);

            dsadasd.Price = window.ConvertPirce2(parseFloat(String(inpPrice.value).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10), window.OldCurrency, "USD");
            estimatedProfitChangeForManuProducts();
            fillPriceTable();
        }
        ...
        $(".slide-2-text-after-first-product").before(div);
        ...
        $(imageDel).click(function () {
            ...
            for (var k = count; k < leng; k++) {
                ...
                if (input) input.id = "Input_" + k;
                if (h4PriceOld) h4PriceOld.id = "h4Price_" + k;
                if (h6PriceOld) h6PriceOld.id = "h6Price_" + k;
                if (h4CostProfRmOld) h4CostProfRmOld.id = "h4CostProfRm_" + k;
                if (h4CostSaleOld) h4CostSaleOld.id = "h4CostSale_" + k;
                if (h4CostProfTextOld) h4CostProfTextOld.id = "h4CostProfText_" + k;
                if (h6CostOld) h6CostOld.id = "h6Cost_" + k;
                if (divProfitCalculOld) divProfitCalculOld.id = "divProfitCalcul_" + k;
                if (h4ProfSaleOld) h4ProfSaleOld.id = "h4ProfSale_" + k;
            }
...
function profitSale() {
    ...
    //var $valNumber = parseFloat(String($val).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10);

    //if($val.length > 5) $val = parseFloat()
    var selPrice =
         window.ConvertPirce2(parseFloat(String($val).match(/-?\d+(?:\.\d+)?/g, '') || 0, 10), window.OldCurrency, "USD");
    //.toFixed(2);

    $(document.getElementById("profSale")).data('original-price', selPrice);


    var price = (selPrice - window.nowPrice);
    ...
    if (selPrice < window.nowPrice) {
        ...
        updateMinimum(price);
        app.state.currentProduct.Price = selPrice; //window.ConvertPirce2(selPrice, window.OldCurrency, "USD"); //window.converPirice2(selPrice);
        window.sellingPrice = app.state.currentProduct.Price;
        if (app.state.products.length > 1) {
            estimatedProfitChangeForManuProducts()
        } else {
    ...
    //$(".currencies-select-box select").change();
    fillPriceTable();
}
...
function Goal() {
    ...
    if (parseInt(app.state.getUsedColorsCountFront()) == parseInt("0") && parseInt(app.state.getUsedColorsCountBack()) == parseInt("0")) {
        ...
        slideTo(2);
        if ($("#profSale").val() && backfrom1 == false) {

        } else {
            setPriceInGoalFromDesign();
        }
        profitSale();
        $("#base-cost-for-first-product").html(window.ConvertPirce2(app.state.currentProduct.BaseCost, "USD", window.OldCurrency).toFixed(2));
        $("#base-cost-for-first-product").data('original-price', app.state.currentProduct.BaseCost);
...
function slideTo(slideNumber) {
    ...
    if (slideNumber == 3) window.seeResultPage = true;
...
function onChangeTrackBar() {
    ...
    window.count = 1; //document.getElementById('trackBarValue').value;//parseInt(document.getElementById('trackbar').value);
    calculatePrice(window.frontColor, window.backColor);
    setPriceInDesignFromGoal();
    var changes = app.state.currentProduct.Price - app.state.currentProduct.BaseCost;
...
function onChangeValueForTrackBar() {
    ...
    window.count = 1; //parseInt(document.getElementById('trackBarValue').value);
    calculatePrice(window.frontColor, window.backColor);
    setPriceInDesignFromGoal();
    //profitSale();
    var changes = app.state.currentProduct.Price - app.state.currentProduct.BaseCost;
