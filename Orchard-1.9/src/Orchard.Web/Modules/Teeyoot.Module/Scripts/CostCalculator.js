...
function calculatePriceForNewProduct(frontColor, backColor, cost) {
    var res = parseFloat(formula(frontColor, backColor, cost, 1));
...
function setPriceInGoalFromDesign() {
    ...
    document.getElementById('base-cost-for-first-product').innerHTML =
                 window.ConvertPirce2(app.state.currentProduct.BaseCost, "USD", window.OldCurrency).toFixed(2);
...
function setPriceInDesignFromGoal() {
    ...
    document.getElementById('count_preview').innerHTML = "Base cost &#64; " + window.count + " item";
}
...
function estimatedProfitChangeForManuProducts() {
    ...
    for (var i = 0; i < products.length; i++) {
        ...
        if (i > 0) {
            var profit = (products[i].Price - products[i].BaseCost);
            ...
            $(indexProf).html(window.ConvertPirce2(profit, "USD", window.OldCurrency).toFixed(2));
            $(indexCost).html(window.ConvertPirce2(products[i].BaseCost, "USD", window.OldCurrency).toFixed(2));
            if (profit <= 0) {
                ...
                $(h4CostProfText).html("Minimum Selling Price: ");
...
function updateMinimum(changes) {
    ...
    if (changes <= 0) {
        ...
        if (app.state.products && app.state.products.length < 2) {
        ...
        //$("#").css('display', 'block');
        $("#base-cost-for-first-product-text").html("Minimum Selling Price: ");
...
function minimumGoal() {
    if (app.state.products != null) {
        ...
        for (var i = 0; i < products.length; i++) {
            ...
            //TYT-23
            count = Math.max(count, nowCount);// ? count : Math.floor(nowCount);
            //End TYT-23
        }
