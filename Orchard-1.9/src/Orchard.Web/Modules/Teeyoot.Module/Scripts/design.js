...
var design={
	...
	ini:function(){
        ...
        $('.size-dialog input').on('change', function () {
            var $this = $(this);
            var item = design.item.get();
            //alert($this.val());
            ratio = $(this).is(':checked');
            var item = design.item.get();
            item.data('keep-ratio', ratio);
        });
        $('.size-dialog input').on('input', function(){
            ...
            if(!isNaN(val) && item.length){
                ...
                design.item.resizeTo(item, isWidth ? val : undefined, !isWidth ? val : undefined, true, design.item.get().data('keep-ratio'));
            }
        });
	...
	text:{
		...
		add: function(o, type, cloneFrom, style_color){
			...
			text.setAttributeNS(null, 'y', parseInt($height));
			...
			svg.setAttributeNS(null, 'width', parseInt($width) + 10);
			svg.setAttributeNS(null, 'height', parseInt($height) + 10);
			svg.setAttributeNS(null, 'viewBox', '0 0 ' + (parseInt($width) + 10) + ' ' + (parseInt($height) + 10));
		...
		setSize: function(e){
			...
			e.css('width', $w + 20 + 'px');
			e.css('height', $h + 20 + 'px');
			var svg = e.find('svg'),
				width = svg[0].getAttribute('width'),
				height = svg[0].getAttribute('height'),
				view = svg[0].getAttribute('viewBox').split(' '),
				vw = (view[2] * $w)/width + 20,
				vh = (view[3] * $h)/height + 20;
			svg[0].setAttributeNS(null, 'width', $w + 20);
			svg[0].setAttributeNS(null, 'height', $h + 20);			
        ...
        resize: function (e, handles, keep) {
			...
			e.resizable({minHeight: 0, minWidth: 15,				
				...
				resizee: function (event, ui) {
				    //keepRatio = $(design.item.get()).data('keep-ratio') || false;
				    //if (keepRatio) side = "scale";
					...
					me.resizeTo(e, $width, $height, false, true);
					oldrect = e[0].getBoundingClientRect();

					//if (keepRatio) {
					//    ui.position.top -= (ui.size.height - oldheight);
					//    ui.position.left -= (ui.size.width - oldwidth);
					//}
				}
			});
