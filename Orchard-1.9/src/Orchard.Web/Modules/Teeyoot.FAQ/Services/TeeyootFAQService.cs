...
namespace Teeyoot.FAQ.Services
{
    public class TeeyootFAQService : ITeeyootFaqService
    {
        ...
        public IEnumerable<FaqSectionRecord> GetFaqSections(string culture)
        {
            //var culture = _workContextAccessor.GetContext().CurrentCulture.Trim();
            //string cultureUsed = culture == "en-SG" ? "en-SG" : (culture == "id-ID" ? "id-ID" : "en-MY");
            return _sectionRepository.Table.Where(s => s.SectionCulture == culture).ToList();
        }

        public FaqSectionRecord GetDefaultSection(string culture)
        {
            //var culture = _workContextAccessor.GetContext().CurrentCulture.Trim();
            //string cultureUsed = culture == "en-SG" ? "en-SG" : (culture == "id-ID" ? "id-ID" : "en-MY");
            return _sectionRepository.Table.Where(s => s.SectionCulture == culture).FirstOrDefault();
        }
