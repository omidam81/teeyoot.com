...
namespace Teeyoot.Account.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        public JsonResult WizardLogOn(string Email, string Password, bool? RememberMe)
        {
            var validRes = ValidateLogOn(Email, Password);
            if (!validRes.IsValid)
            {
                return Json(new JsonResponseBase {Message = validRes.ValidationSummary});
            }

            _authenticationService.SignIn(validRes.User, RememberMe.HasValue ? RememberMe.Value : false);
        ...
        {
            nonce = HttpUtility.UrlDecode(nonce); //nonce.Replace(" ", "+");
        ...
        public ActionResult ResetPassword(ResetPasswordViewModel viewModel)
        {
            ...
            var user = _userService.ValidateLostPassword(HttpUtility.UrlDecode(viewModel.Nonce));
            if (user == null)
            {
                return Redirect("~/");
            }
